...
export const parsePackageSelection = (args) => {
  if (args.length !== 2 || args[0] !== "--package" || args[1] !== packageName) {
    throw new Error(
      `사용법: npm run test-packed-package -- --package ${packageName}`,
    );
  }
  return packageName;
};
...
const main = async (args = process.argv.slice(2)) => {
  ...
  const tmpRoot = await mkdtemp(
    join(tmpdir(), "packed-next-webpack-baseline-"),
  );
  try {
    ...
    for (const file of ["README.md", "LICENSE"])
      await stat(
        join(consumerDir, "node_modules/@cp949/next-webpack-baseline", file),
      );
